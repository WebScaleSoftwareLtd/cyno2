...
export default async function() {
    ...
    if (Date.now() > token[2]) {
        ...
        if (!process.env.DISCORD_CLIENT_ID || !process.env.DISCORD_CLIENT_SECRET) {
            throw new Error("DISCORD_CLIENT_ID or DISCORD_CLIENT_SECRET are missing.");
        }
        ...
        token = [j.access_token, j.refresh_token, Date.now() + j.expires_in * 1000];
        cookies().set("encrypted_token", vault.encrypt(JSON.stringify(token)).toString(), {
