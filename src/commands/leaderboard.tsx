...
const Leaderboard = ({ initPageResult, pageLoader }: Props) => {
    ...
    if (page.result.fields.length === 0) {
        return <Embed
            description="There is currently nobody on this leaderboard."
        />;
    }
    ...
    return <>
        ...
        {
            page.page !== 1 && <Button
                label="Previous Page" onClick={() => renderPage(page.page - 1)}
                emoji="⬅️"
            />
        }
        {
            !page.result.endPage && <Button
                label="Next Page" onClick={() => renderPage(page.page + 1)}
                emoji="➡️"
            />
        }
    </>;
...
const getUser = (
    id: string, interaction: CommandInteraction,
): User | null => {
    ...
    let pageLoader: (page: number) => Promise<PageResult> = async page => {
        ...
        return {
            fields: res.map((w, i) => {
                ...
                if (userCache) name = `${num}) ${userCache.username}${userCache.discriminator === "0" ? "" : `#${userCache.discriminator}`}`;
    
                // Return the embed field.
                return {
                    name, value: `${guild.currencyEmoji} ${w.balance}`,
                    inline: true,
                };
            }),
            endPage,
        ...
        pageLoader = async page => {
            ...
            return {
                fields: res.map((x, i) => {
                    ...
                    // Return the embed field.
                    return {
                        name, value: `Level ${x.level} (${x.xp} XP)`,
                        inline: true,
                    };
                }),
                endPage,
