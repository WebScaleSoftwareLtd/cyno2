...
const loadShares = async (gid: bigint, uid: bigint, pageNumber: number): Promise<DisplayedShares> => {
    // Keep going until page 1 || shares are found.
    for (;;) {
        // Load the shares from the database.
        const shares = await client.query.shares.findMany({
            where: (tx, { and, eq }) => and(
                eq(tx.guildId, gid),
                eq(tx.userId, uid),
            ),
        ...
        const sharePromises = await Promise.all([...new Set(shares.map(share => share.stockName))].map(async stockName => {
            return (async () =>
                [stockName, await yahooFinance.quote(stockName, { fields: ["regularMarketPrice", "shortName"] })] as const
            )().catch(() => [stockName, undefined] as const);
        }));

        // If shares are empty, check if we can go back a page.
        if (shares.length === 0) {
            if (pageNumber === 1) return { page: [], pageNumber: 1, endPage: true };
            pageNumber--;
            continue;
        }
        ...
        return {
            page: shares.map(share => {
                const shareData = sharePromises.find(([stockName]) => stockName === share.stockName)!;
                const shareLabel = shareData[1]?.shortName;
                const value = shareData[1] ? Math.floor(share.shareCount * shareData[1].regularMarketPrice!) : share.invested;
...
type ViewState = { shares: DisplayedShares } | { share: ShareValue, pageNumber: number };
...
const renderShareDescription = (emoji: string, tx: ShareValue) => `Bought ${tx.shareCount} shares at ${emoji} ${tx.invested}

**Current Value**: ${emoji} ${tx.value}`;

const SharesPage = ({ displayedShares, setView, uid, gid, emoji }: PageProps) => <>
    <Embed
    ...
    {
        displayedShares.pageNumber !== 1 && <Button
            label="Previous Page" onClick={async () => setView({
                shares: await loadShares(gid, uid, displayedShares.pageNumber - 1),
            })} emoji="⬅️"
        />
    }
    {
        !displayedShares.endPage && <Button
            label="Next Page" onClick={async () => setView({
                shares: await loadShares(gid, uid, displayedShares.pageNumber + 1),
            })} emoji="➡️"
        />
    }
    ...
    {
        displayedShares.page.map((tx, i) => <Button
            label={`Manage Share ${i + 1}`}
            onClick={() => setView({ share: tx, pageNumber: displayedShares.pageNumber })}
            key={i}
        />)
    }
</>;
...
const ManageShare = ({ uid, gid, share, pageNumber, setView, emoji }: ShareProps) => {
    ...
    return <>
        <Embed
            description={renderShareDescription(emoji, share)}
        />

        <Button
            label="Show Shares" onClick={async () => {
                setView({ shares: await loadShares(gid, uid, pageNumber) });
            }}
            emoji="🔼"
        />
...
const Router = ({ uid, gid, emoji, initialPage }: Props) => {
    ...
    if ("shares" in state) {
        ...
        if (state.shares.page.length === 0) {
            return <Embed
                description="You currently have no shares."
            />;
        }
